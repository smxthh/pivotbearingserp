...
export default function PurchaseReport() {
  ...
  const totalPurchases = filteredInvoices.reduce((sum, inv) => sum + inv.grandTotal, 0);
  ...
  return (
    <PageContainer title="Purchase Report">
      {/* Filters */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-card rounded-lg border p-4">
          ...
          <p className="text-2xl font-bold text-destructive mt-1">{formatCurrency(totalPurchases)}</p>
        </div>
        <div className="bg-card rounded-lg border p-4">
      ...
      <DataTable
        ...
        keyExtractor={(i) => i.id}
        emptyMessage="No purchases in selected period"
      />
    </PageContainer>
  );
}
