...
export default function SalesOrderPage() {
    ...
    const handleExport = () => {
        exportToCSV(
            filteredVouchers,
            [
                { key: 'voucher_number', header: 'Order No.' },
                { key: 'voucher_date', header: 'Order Date', render: (v) => formatDateForExport(v.voucher_date) },
                { key: 'delivery_date', header: 'Delivery Date', render: (v: any) => v.delivery_date ? formatDateForExport(v.delivery_date) : '-' },
                { key: 'party_name', header: 'Customer Name', render: (v) => v.party_name || '-' },
                { key: 'customer_po_number', header: 'Customer PO', render: (v: any) => v.customer_po_number || '-' },
    ...
    const columns: Column<Voucher>[] = [
        ...
        { key: 'voucher_number', header: 'Order No.', render: (v) => <span className="font-mono font-medium text-primary">{v.voucher_number}</span> },
        { key: 'voucher_date', header: 'Order Date', render: (v) => formatDate(v.voucher_date) },
        { key: 'delivery_date', header: 'Delivery Date', render: (v: any) => v.delivery_date ? formatDate(v.delivery_date) : '-' },
        { key: 'party_name', header: 'Customer Name', render: (v) => v.party_name || '-' },
    ...
    if (isLoading) return <PageContainer title="Sales Order"><div className="space-y-4"><Skeleton className="h-10 w-64" /><div className="bg-card rounded-xl border">{[...Array(8)].map((_, i) => <Skeleton key={i} className="h-12 m-2" />)}</div></div></PageContainer>;

    return (
        <PageContainer title="Sales Order">
            <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v as any); setCurrentPage(1); }}>
                ...
                <TabsContent value="cancelled" className="mt-0">
                    <DataTable columns={columns} data={paginatedVouchers} keyExtractor={(v) => v.id} emptyMessage="No cancelled orders" onRowClick={handleView} />
                </TabsContent>
            ...
            <SalesOrderDialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen} />
            {selectedVoucher && <VoucherViewDialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen} voucherId={selectedVoucher.id} />}
            <ConfirmDialog open={isCancelDialogOpen} onOpenChange={setIsCancelDialogOpen} title="Cancel Sales Order" description={`Cancel "${selectedVoucher?.voucher_number}"?`} confirmLabel="Cancel" onConfirm={confirmCancel} isLoading={isCancelling} variant="destructive" />
        </PageContainer>
    );
}
