...
export default function DeliveryChallanPage() {
    ...
    const handleExport = () => {
        exportToCSV(
            filteredVouchers,
            [
                { key: 'voucher_number', header: 'Challan No.' },
                { key: 'voucher_date', header: 'Challan Date', render: (v) => formatDateForExport(v.voucher_date) },
                { key: 'party_name', header: 'Customer Name', render: (v) => v.party_name || '-' },
                { key: 'ship_to', header: 'Ship To', render: (v: any) => v.ship_to || '-' },
                { key: 'transport_name', header: 'Transport', render: (v: any) => v.transport_name || '-' },
                { key: 'lr_number', header: 'LR No.', render: (v: any) => v.lr_number || '-' },
                { key: 'po_number', header: 'Sales Order', render: (v: any) => v.po_number || '-' },
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'confirmed': return <Badge className="bg-green-500/20 text-green-600 border-green-500/30">Delivered</Badge>;
    ...
    const columns: Column<Voucher>[] = [
        ...
        { key: 'voucher_number', header: 'Challan No.', render: (v) => <span className="font-mono font-medium text-primary">{v.voucher_number}</span> },
        { key: 'voucher_date', header: 'Challan Date', render: (v) => formatDate(v.voucher_date) },
        { key: 'party_name', header: 'Customer Name', render: (v) => v.party_name || '-' },
        { key: 'ship_to', header: 'Ship To', render: (v: any) => v.ship_to || '-' },
        { key: 'transport_name', header: 'Transport', render: (v: any) => v.transport_name || '-' },
        { key: 'lr_number', header: 'LR No.', render: (v: any) => v.lr_number || '-' },
    ...
    if (isLoading) return <PageContainer title="Delivery Challan"><div className="space-y-4"><Skeleton className="h-10 w-64" /><div className="bg-card rounded-xl border">{[...Array(8)].map((_, i) => <Skeleton key={i} className="h-12 m-2" />)}</div></div></PageContainer>;

    return (
        <PageContainer title="Delivery Challan">
            <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v as any); setCurrentPage(1); }}>
                <div className="flex items-center justify-between mb-6">
                    ...
                    <Button onClick={() => setIsCreateDialogOpen(true)}>
                        <Plus className="h-4 w-4 mr-2" />Add Challan
                    </Button>
                </div>

                <h2 className="text-lg font-semibold mb-4">Delivery Challan</h2>
                ...
                <TabsContent value="cancelled" className="mt-0">
                    <DataTable columns={columns} data={paginatedVouchers} keyExtractor={(v) => v.id} emptyMessage="No cancelled challans" onRowClick={handleView} />
                </TabsContent>
            ...
            <DeliveryChallanDialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen} />
            {selectedVoucher && <VoucherViewDialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen} voucherId={selectedVoucher.id} />}
            <ConfirmDialog open={isCancelDialogOpen} onOpenChange={setIsCancelDialogOpen} title="Cancel Delivery Challan" description={`Cancel "${selectedVoucher?.voucher_number}"?`} confirmLabel="Cancel" onConfirm={confirmCancel} isLoading={isCancelling} variant="destructive" />
        </PageContainer>
    );
}
