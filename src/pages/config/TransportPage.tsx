...
export default function TransportPage() {
    ...
    // Data fetching
    const { transports, totalCount, isLoading, refetch, deleteTransport, profile } = useTransports({
        page: currentPage,
        pageSize,
    ...
    const handleConfirmDelete = async () => {
        if (recordToDelete) {
            await deleteTransport.mutateAsync(recordToDelete.id);
    ...
    const handleExportExcel = async () => {
        ...
        const exportData = data.map((record: any, index: number) => ({
            '#': index + 1,
            'Transport Name': record.transport_name,
        ...
        XLSX.writeFile(wb, 'transport.xlsx');
    };
    ...
    return (
        <PageContainer title="Transport" actions={headerActions}>
            <div className="space-y-6">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-primary hover:bg-primary">
                                ...
                                <TableHead className="text-primary-foreground text-xs">Transport Name</TableHead>
                                <TableHead className="text-primary-foreground text-xs">Transport ID</TableHead>
                                <TableHead className="text-primary-foreground text-xs">Address</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {transports.length === 0 ? (
                                ...
                                transports.map((record, index) => (
                                    <TableRow key={record.id} className="hover:bg-muted/30">
                                        ...
                                        <TableCell className="text-xs font-medium text-primary">{record.transport_name}</TableCell>
                                        <TableCell className="text-xs text-slate-600">{record.transport_id}</TableCell>
                                        <TableCell className="text-xs text-slate-600 max-w-[300px] truncate">{record.address || '-'}</TableCell>
                                    </TableRow>
                                ))
                            )}
                        </TableBody>
                ...
                <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">
                        Showing {transports.length === 0 ? 0 : ((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, totalCount)} of {totalCount} entries
                    </span>
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will permanently delete this transport. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
