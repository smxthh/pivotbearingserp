...
import { DebitNoteDialog } from '@/components/accounting/DebitNoteDialog';
...
export default function DebitNotePage() {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'confirmed': return <Badge className="bg-success">Confirmed</Badge>;
    ...
    const columns: Column<Voucher>[] = [
        {
            ...
            render: (voucher) => (
                <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); handleView(voucher); }} className="h-8 w-8">
                        <Eye className="h-4 w-4 text-primary" />
                    </Button>
                    {voucher.status === 'confirmed' && (
                        <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); handleCancel(voucher); }} className="h-8 w-8">
        ...
        { key: 'dn_type', header: 'DN Type', render: (v: any) => v.dn_type || 'Decrease Purchase' },
        { key: 'voucher_number', header: 'DN No.', render: (v) => <span className="font-mono font-medium">{v.voucher_number}</span> },
        { key: 'voucher_date', header: 'DN Date', render: (v) => formatDate(v.voucher_date) },
        { key: 'party_name', header: 'Party Name', render: (v) => v.party_name || '-' },
        { key: 'subtotal', header: 'Taxable Amount', render: (v) => formatCurrency(v.subtotal || 0), className: 'text-right' },
    ...
    if (isLoading) {
        return (
            <PageContainer title="Debit Note">
                <div className="space-y-4">
                    <Skeleton className="h-10 w-64" />
                    <div className="bg-card rounded-xl border">
                        {[...Array(8)].map((_, i) => <Skeleton key={i} className="h-12 m-2" />)}
                    </div>
                </div>
            </PageContainer>
        );
        ...
        <PageContainer title="Debit Note">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button onClick={() => setIsCreateDialogOpen(true)}><Plus className="h-4 w-4 mr-2" />Add Debit Note</Button>
                </div>
            </div>

            <DataTable columns={columns} data={paginatedVouchers} keyExtractor={(v) => v.id} emptyMessage="No debit notes found" onRowClick={handleView} />
            ...
            <DebitNoteDialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen} />
            {selectedVoucher && <VoucherViewDialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen} voucherId={selectedVoucher.id} />}
            <ConfirmDialog open={isCancelDialogOpen} onOpenChange={setIsCancelDialogOpen} title="Cancel Debit Note" description={`Cancel "${selectedVoucher?.voucher_number}"?`} confirmLabel="Cancel" onConfirm={confirmCancel} isLoading={isCancelling} variant="destructive" />
        </PageContainer>
    );
}
