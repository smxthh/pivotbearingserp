...
import { useMarking, MarkingBatch } from '@/hooks/useMarking';
...
const formSchema = z.object({
    ...
    mrk_date: z.string().min(1, 'Required'),
...
interface AddMarkingDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export function AddMarkingDialog({
    open,
    onOpenChange,
}: AddMarkingDialogProps) {
    const { createMarking, getNextNumber } = useMarking();
    const { items } = useItems();
    const { storeLocations } = useStoreLocations();
    const { salesExecutives } = useSalesExecutives();

    // Fetch prefixes
    const { prefixes: dbPrefixes, defaultPrefix: dbDefaultPrefix } = useVoucherPrefixesForType('Marking');
    ...
    const {
        ...
        defaultValues: {
            doc_prefix: 'MRK/25-26/',
            doc_number: 1,
            mrk_date: new Date().toISOString().split('T')[0],
        },
    });
    ...
    const loadAvailableBatches = async () => {
        ...
        try {
            // Fetch available stock from view_batch_stock_ledger
            const { data, error } = await (supabase
                .from('view_batch_stock_ledger' as any) as any)
                .select(`
                    batch_number,
                    quantity,
                    location_id
            ...
            const formattedBatches = (data || []).map((item: any) => ({
                id: `${item.batch_number}-${item.location_id}`, // Generate a unique ID since view doesn't have one
                gi_number: '', // Not available in ledger view, not critical for selection
                batch_number: item.batch_number,
                location_id: item.location_id,
                location_name: storeLocations?.find(l => l.id === item.location_id)?.store_name || '', // Map name client-side
    ...
    // Auto-fetch next number when prefix changes
    useEffect(() => {
        let active = true;
        const fetchNum = async () => {
            if (open && watchedPrefix && getNextNumber) {
                const num = await getNextNumber(watchedPrefix);
                if (active) {
        ...
        fetchNum();
        return () => { active = false; };
    }, [open, watchedPrefix, getNextNumber, setValue]);
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            await createMarking.mutateAsync({
                // Pass prefix explicitly for the RPC
                ...data,
                mrk_number: data.doc_number,
                mrk_full_number: `${data.doc_prefix}${data.doc_number}`,
                ...
                // Extra field for RPC
                mrk_prefix: data.doc_prefix,
            } as any); // Cast to any because CreateMarkingData doesn't strictly have mrk_prefix yet but RPC needs it
            ...
            <DialogContent className="sm:max-w-[1000px] p-0 overflow-hidden gap-0">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-6 overflow-y-auto max-h-[80vh]">
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <DatePicker
                                            ...
                                            className={errors.mrk_date ? 'border-destructive' : ''}
                                        />
                                    )}
                                />
                            </div>
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="bg-primary hover:bg-primary">
                                            ...
                                            <TableHead className="text-primary-foreground text-xs h-9 text-right">Marking Qty</TableHead>
                    ...
                    <DialogFooter className="mt-8">
                        ...
                        <Button
                            type="submit"
                            disabled={createMarking.isPending || batches.length === 0}
                            className="px-8"
                        >
                            {createMarking.isPending ? 'Saving...' : 'Save Marking'}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
