...
const formSchema = z.object({
    doc_prefix: z.string().default('INC/25-26/'),
...
interface GSTIncomeDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export function GSTIncomeDialog({ open, onOpenChange }: GSTIncomeDialogProps) {
    ...
    const { prefixes: dbPrefixes, defaultPrefix } = useVoucherPrefixesForType('GST Income');
    ...
    const isInterState = watchedGstType === 'GST Inter-State Sales' || watchedGstType === 'GST Exports';
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            // Use manually entered income document number
            const voucherNumber = `${data.doc_prefix}${data.doc_number}`;
            const party = parties.find(p => p.id === data.party_id);

            // Build ledger postings - for income, we credit income accounts
            const ledgerPostings: LedgerPostingItem[] = entries.map(entry => ({
                ledger_id: entry.ledger_id,
                debit_amount: 0,
                credit_amount: entry.amount + entry.cgst_amount + entry.sgst_amount,
                narration: `GST Income - ${entry.ledger_name}`,
            }));

            // Debit party account (receivable)
            ...
            if (partyLedger) {
                ledgerPostings.push({
                    ...
                    credit_amount: 0,
                    narration: `GST Income from ${party?.name}`,
                });
            }

            // Map to voucher object with required fields, casting to any to allow custom fields if backend supports or for simple bypass
            const voucherPayload = {
                ...
                status: 'confirmed',
                reference_number: data.po_challan_no
            } as any;
            ...
            console.error('Error creating GST income:', error);
        }
    };
                        ...
                        <div className="col-span-5 space-y-2">
                            ...
                            {selectedParty && (
                                <div className="flex justify-between text-xs text-muted-foreground">
                                    <span>Cl. Balance: <span className="font-medium">{formatCurrency(selectedParty.current_balance || 0)}</span></span>
                                </div>
                            )}
                        </div>
