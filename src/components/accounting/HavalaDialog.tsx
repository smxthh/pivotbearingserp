...
const formSchema = z.object({
    doc_prefix: z.string().default('HAV/25-26/'),
    doc_number: z.coerce.number().min(1, 'Required'),
    voucher_date: z.string().min(1, 'Required'),
    master_ledger_id: z.string().min(1, 'Master Ledger is required'),
});

type FormData = z.infer<typeof formSchema>;

interface HavalaDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export function HavalaDialog({ open, onOpenChange }: HavalaDialogProps) {
    ...
    const { prefixes: dbPrefixes, defaultPrefix } = useVoucherPrefixesForType('Havala');
    ...
    const [entries, setEntries] = useState<HavalaTableEntry[]>([]);
    ...
    const onSubmit = async (data: FormData) => {
        ...
        // Calculate net balance needed for Master Ledger
        const netTableDebit = totals.totalDebit - totals.totalCredit;
        const masterAmount = Math.abs(netTableDebit);
        const masterType = netTableDebit > 0 ? 'Credit' : 'Debit';
        ...
        try {
            ...
            // Add Master Ledger entry to balance
            if (masterAmount > 0) {
                ledgerPostings.push({
                    ledger_id: data.master_ledger_id,
                    debit_amount: masterType === 'Debit' ? masterAmount : 0,
                    credit_amount: masterType === 'Credit' ? masterAmount : 0,
                    narration: 'Havala Contra Entry',
                });
            }
            ...
            await createVoucher.mutateAsync({
                voucher: {
                    ...
                    narration: 'Havala Entry',
                    total_amount: Math.max(totals.totalDebit, totals.totalCredit),
                    status: 'confirmed',
                },
                items: [],
                ledgerPostings,
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto">
                <DialogHeader className="bg-primary text-primary-foreground -mx-6 -mt-6 px-6 py-4 rounded-t-lg">
                    <DialogTitle className="text-lg font-semibold">Havala Entry</DialogTitle>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 pt-2">
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <DatePicker
                                        ...
                                        className={errors.voucher_date ? 'border-destructive text-xs' : 'text-xs'}
                                    />
                                )}
                            />
                        </div>

                        {/* Ledger Name - 6 columns */}
                        <div className="col-span-6 space-y-2">
                            <Label className="text-sm">
                                Master Ledger <span className="text-destructive">*</span>
                            </Label>
                            <Select
                                value={watch('master_ledger_id')}
                                onValueChange={v => setValue('master_ledger_id', v)}
                            >
                                <SelectTrigger className={`text-xs ${errors.master_ledger_id ? 'border-destructive' : ''}`}>
                    ...
                    <div className="space-y-3">
                        <Label className="text-sm font-medium">Havala Details :</Label>
                    ...
                    {/* Balance Info */}
                    <div className={`flex items-center gap-2 p-3 rounded-lg ${totals.isBalanced ? 'bg-green-100 border border-green-300' : 'bg-blue-100 border border-blue-300'}`}>
                        {totals.isBalanced ? (
                            <>
                                <CheckCircle className="h-5 w-5 text-green-600" />
                                <span className="text-sm font-medium text-green-700">
                                    Entries are balanced (Master ledger will auto-balance)
                                </span>
                            </>
                        ) : (
                            <>
                                ...
                                <span className="text-sm font-medium text-blue-700">
                                    Net Difference: {formatCurrency(totals.difference)} will be posted to Master Ledger
                                </span>
                            </>
                        )}
                    </div>
                    ...
                    <DialogFooter className="gap-2">
                        ...
                        <Button
                            type="submit"
                            disabled={isCreating || entries.length === 0 || !watch('master_ledger_id')}
                        >
